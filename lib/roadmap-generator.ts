...
function extractPrerequisites(metadata: string): string {
  ...
  const prereqMatch =
    metadata.match(/prerequisite[s]?:\s*([^;.]+)/i) ||
...
1. **RESOLVE COURSE CHOICES**: When a course field contains multiple options (using "or", "/", "and", or commas), select EXACTLY ONE course.
  - Replace the entire field with only the chosen course code (e.g., "MATH 215 or 241 or 251A" becomes "MATH 241")
...
2. **REPLACE ELECTIVES AND PLACEHOLDERS**: Replace any course containing "Elective", "400+", "300+", or similar level indicators with an actual course from the available courses list.
  - For "${majorPrefix} 400+" or "${majorPrefix} 300+" electives, choose from the corresponding level in the available courses
  - For generic "Elective" without a department, choose from 400+ level courses in the major
  - Select courses that align with the career goal, skills, and interests
  - When you encounter placeholders like 'Elective', '${majorPrefix} 400+', '${majorPrefix} 300+', or similar department-level placeholders:
    - Replace the placeholder with an EXACT course code from the AVAILABLE COURSES list included above (the course code must appear verbatim in the AVAILABLE COURSES block). Do NOT invent course codes.
    - Prefer courses whose title or metadata contains the student's skills, interests, or career goal (case-insensitive exact phrase or clear synonym).
    - If multiple valid candidates exist, prefer the one with explicit metadata/prerequisite fit
    - If no suitable candidate exists in the AVAILABLE COURSES list, choose any appropriate level course from the major department
  - CRITICAL: Track all courses you've selected. NEVER use the same course code more than once in the entire pathway
  - When you replace an Elective or department-level placeholder with a concrete course, you MUST include an 'isRelated' array on that chosen course listing indicating which profile item(s) (type: 'skill'|'interest'|'career' and value: exact text) motivated the selection. Only include entries when catalog evidence or department prefix supports the choice.
...
7. **ADD ACTIVITIES AND MILESTONES**: For EVERY semester object, you MUST include two arrays: 'activities' and 'milestones'.
  - 'activities': an array of at least three actionable, concrete items the student should do that semester
  - 'milestones': an array of at least three specific academic or career achievements to complete that semester
  - Both arrays should be present for EVERY semester.
  - Make these relevant to the student's career goal, the courses they're taking that semester, interests, and skills.
  - Use complete, actionable sentences.

8. **MARK RELATED COURSES**: For EVERY course object you MUST include a field named "isRelated". This field MUST be either null or an ARRAY of one or more objects. Each ARRAY object MUST have exactly these properties:
  - 'type' (string): one of the exact values 'skill', 'interest', or 'career'.
  - 'value' (string): the exact skill name or interest text (or short career phrase) from the student's profile that this course matches.

  Rules:
  - If a course matches multiple profile items, include multiple objects in the array.
  - Use 'career' for direct career fits (you may include it alongside skills/interests when appropriate).
  - Use only the exact 'type' values 'skill', 'interest', or 'career'.
  - ONLY mark a course as related if there is CLEAR evidence: the course catalog title/description explicitly mentions the skill/interest/career term, OR the course is an advanced elective you specifically chose to match that profile item.
  - DO NOT mark foundational/required courses as related unless the course title/description clearly matches. For example, do not mark "ICS 141 (FQ)" as related to "Web Development" - it's a discrete mathematics course.
  - DO NOT guess at relationships. When in doubt, set isRelated to null.
  - ONLY mark courses as related when: (a) you specifically selected them from available courses to match a profile item, OR (b) the course catalog explicitly mentions the profile term.


CRITICAL: 
- Ensure EVERY semester has both 'activities' and 'milestones' arrays populated with relevant items
- NEVER reuse the same course code - track every course you select and ensure uniqueness
- ALL course choices (those with "or", "/", etc.) must be resolved to a single course code
- ALL placeholders ("Elective", "DEPT 400+", etc.) must be replaced with actual course codes
- The pathway should guide the student's entire college experience, not just their coursework

Return the COMPLETE modified pathway as valid JSON. Keep ALL years, ALL semesters, ALL courses, and include 'activities' and 'milestones' arrays for every semester.`;
...
export async function generateAndSaveRoadmap(profileId: number): Promise<void> {
  ...
  const processedRoadmap = await processPathwayWithAI(matchingPathway, relevantCourses, profileData, majorPrefix);
